...
class DisjointSet(object):
    """Simulates a sequence of merge operations with 
    tables in a database.
    """
    def __init__(self, n, lines):
        """Initializes a set for given n elements.
        Initially, the set consists of one element 
        which is pointing to itself.
        Also during initialization the rank(tree's 
        height) is assigned to 1
        for each set."""
    ...
    def get_parent(self, x):
        """Finds a set id (root of the tree) for 
        element x and compresses path.
        """
        parents_to_update = []
    ...
    def merge(self, dest, src):
        """Unions tables.
        During union updates rank's(tree's height) 
        array."""
